...
raw$Exited %>% table() %>% prop.table() #imbalance problemi var

raw %>% inspect_na()   #na yoxdur 

raw$HasCrCard<- raw$HasCrCard %>% as.character()
raw$Exited<- raw$Exited %>% as.character()
raw$IsActiveMember<- raw$IsActiveMember %>% as.character()
raw$NumOfProducts<- raw$NumOfProducts %>% as.character()
...
# One Hote Encoding
dummy<- dummyVars(" ~ .", data = df.chr[,-1]) %>% 
  predict(newdata = df.chr[,-1]) %>% 
  as.data.frame()


df <- cbind(df.chr$Exited,dummy,df.num) 

colnames(df)[1]= "Exited"
...
# Exclude not important variables    ## weight hadisesi 
ivars <- iv %>% 
...
while(glm %>% vif() %>% arrange(desc(gvif)) %>% .[1,2] >= 1.5){
  afterVIF <- glm %>% vif() %>% arrange(desc(gvif)) %>% .[-1,"variable"] %>% .[[-1]]
